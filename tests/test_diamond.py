...
def _assert_certificate_feasible(certificate, atol=2e-5):
...
def test_identity_map_has_unit_diamond_norm():
    certificate = diamond_norm_from_choi(
        identity_choi(2),
        2,
        2,
        solver=SOLVER,
    ...
    assert np.isclose(certificate.diamond_norm, 1.0, atol=2e-5)
...
def test_identical_channels_have_zero_diamond_distance():
    ...
    _assert_certificate_feasible(certificate)
    assert certificate.diamond_norm <= 2e-5
...
def test_identity_is_distance_three_halves_from_closest_constant_qubit_channel():
    certificate = closest_constant_channel_diamond_distance(
        identity_channel(2),
        solver=SOLVER,
    )
    _assert_certificate_feasible(certificate)
    assert certificate.constant_state_trace_residual <= 2e-5
    assert certificate.minimum_constant_state_eigenvalue >= -2e-5
    assert np.allclose(certificate.constant_state, np.eye(2) / 2.0, atol=3e-5)
    assert np.isclose(certificate.diamond_distance, 1.5, atol=3e-5)
...
def test_optimal_dephasing_recovery_has_analytic_diamond_error(
    ...
    certificate = optimal_recovery_diamond_error(
        dephasing_channel(probability),
        solver=SOLVER,
    )
    _assert_certificate_feasible(certificate)
    assert certificate.recovery_trace_preservation_residual <= 2e-5
    assert certificate.minimum_recovery_choi_eigenvalue >= -2e-5
    assert np.isclose(certificate.diamond_error, expected, atol=4e-5)
...
def test_ksw_tradeoff_holds_for_dephasing(probability):
    ...
    _assert_certificate_feasible(certificate.recovery, atol=3e-5)
    _assert_certificate_feasible(certificate.environment, atol=3e-5)
    assert certificate.lower_margin >= -5e-5
    assert certificate.upper_margin >= -5e-5
